			...
			<head>
				<xsl:choose>
					<xsl:when test="kml:kml">
						<title>Sơ đồ trang web địa điểm - TheKim (american design)</title>
					</xsl:when>
					<xsl:otherwise>
						<title>Sơ đồ trang web XML - TheKim (american design)</title>
					</xsl:otherwise>
				</xsl:choose>
				...
				<xsl:choose>
					<xsl:when test="kml:kml">
						...
						<div id="content">
							...
							<p class="expl">
								<a href="https://https://github.com/211591NguyenTrongHuu/WP-WhiteCode.git/sitemap_index.xml">← Chỉ mục Sơ đồ trang web</a>							</p>
